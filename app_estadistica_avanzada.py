...
def calcular_correlacion(df, variables, metodo='pearson'):
    ...
    try:
        ...
        if len(vars_numericas) < 2:
            return None, f"Se necesitan al menos 2 variables numéricas. Variables válidas: {vars_numericas}"
...
def crear_visualizacion_contingencia(df, var1, var2, tipo='barras'):
    ...
    if tipo == 'barras':
        fig = px.bar(
            ...
            x=var1, y='Frecuencia', color=var2,
    ...
    for columna, filtro in filtros.items():
        if filtro['tipo'] == 'rango':
            ...
            df_filtrado = df_filtrado[
                (df_filtrado[columna] >= min_val) & 
                (df_filtrado[columna] <= max_val)
            ]
        elif filtro['tipo'] == 'categorias':
            categorias_seleccionadas = filtro['valores']
            df_filtrado = df_filtrado[df_filtrado[columna].isin(categorias_seleccionadas)]
...
def crear_visualizacion_avanzada(df, tipo, variables, **kwargs):
    ...
    if tipo == 'boxplot':
        if len(variables) == 1:
            ...
            fig = px.box(df, x=variables[1], y=variables[0], title=f'Boxplot de {variables[0]} por {variables[1]}')
    
    elif tipo == 'scatter':
        fig = px.scatter(
            df, x=variables[0], y=variables[1],
        ...
        if len(variables) == 1:
            ...
            fig = px.violin(df, x=variables[1], y=variables[0], title=f'Violin plot de {variables[0]} por {variables[1]}')
...
def exportar_resultados(df, resultados_corr, resultados_cont, formato='csv'):
    ...
    if formato == 'csv':
        ...
        output.write(f"Fecha de análisis: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
...
st.markdown('<h1 class="main-header">📊 Análisis Estadístico Avanzado</h1>', unsafe_allow_html=True)
st.write("Aplicación para análisis estadístico avanzado con filtros dinámicos, correlaciones, tablas de contingencia y visualizaciones.")
...
with st.sidebar:
    ...
    if st.session_state.df is not None:
        ...
        vars_numericas = tipos_df[tipos_df['tipo_detectado'].str.contains('numérico', na=False)]['columna'].tolist()
        vars_categoricas = tipos_df[tipos_df['tipo_detectado'] == 'categórico']['columna'].tolist()
...
if st.session_state.df is not None:
    st.markdown('<h2 class="section-header">🔍 Filtros Dinámicos</h2>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        for var in vars_categoricas:
            ...
            st.session_state.filtros_aplicados[var]['valores'] = categorias_seleccionadas
    ...
    if st.session_state.df_filtrado is not None and len(st.session_state.df_filtrado) != len(df):
        st.info(f"📊 **Datos filtrados:** {len(st.session_state.df_filtrado):,} de {len(df):,} registros")
    ...
    if len(vars_numericas) >= 2:
        ...
        if len(variables_seleccionadas) >= 2:
            if st.button("📊 Calcular Correlación", type="primary"):
                corr_matrix, error = calcular_correlacion(df_actual, variables_seleccionadas, metodo_correlacion)
                ...
                if error:
                    ...
                    # Crear heatmap
                    fig_heatmap = crear_heatmap_correlacion(corr_matrix, f"Matriz de Correlación ({metodo_correlacion.title()})")
        ...
        st.warning("⚠️ Se necesitan al menos 2 variables numéricas para el análisis de correlación")
...
if st.session_state.df_filtrado is not None:
    st.markdown('<h2 class="section-header">📊 Tablas de Contingencia</h2>', unsafe_allow_html=True)
    ...
    if len(vars_categoricas) >= 2:
        ...
        with col1:
            var1 = st.selectbox("Variable 1:", options=vars_categoricas, key="cont_var1")
        
        with col2:
            var2 = st.selectbox("Variable 2:", options=vars_categoricas, key="cont_var2")
        ...
        if var1 != var2:
            if st.button("📊 Crear Tabla de Contingencia", type="primary"):
                ...
                if resultados:
                    ...
                    st.dataframe(resultados['tabla_porcentaje'].round(2), use_container_width=True)
                    ...
                    tipo_vis = st.selectbox("Tipo de visualización:", ["barras", "heatmap"])
                    
                    fig_vis = crear_visualizacion_contingencia(df_actual, var1, var2, tipo_vis)
                    st.plotly_chart(fig_vis, use_container_width=True)
                else:
                    st.error("❌ Error al crear la tabla de contingencia")
    else:
        st.warning("⚠️ Se necesitan al menos 2 variables categóricas para las tablas de contingencia")
...
if st.session_state.df_filtrado is not None:
    st.markdown('<h2 class="section-header">🎨 Visualizaciones Avanzadas</h2>', unsafe_allow_html=True)
    ...
    with col1:
        tipo_visualizacion = st.selectbox(
            ...
            options=[
                "boxplot", "scatter", "density", "violin", 
                "scatter_matrix", "heatmap_avanzada"
            ],
    ...
    with col2:
        if tipo_visualizacion in ["scatter", "scatter_matrix", "heatmap_avanzada"]:
            if tipo_visualizacion == "scatter":
                if len(vars_numericas) >= 2:
                    var_x = st.selectbox("Variable X:", options=vars_numericas, key="vis_x")
                    var_y = st.selectbox("Variable Y:", options=vars_numericas, key="vis_y")
            ...
            var_y = st.selectbox("Variable numérica:", options=vars_numericas, key="box_y")
            if vars_categoricas:
                var_x = st.selectbox("Variable categórica (opcional):", options=["Ninguna"] + vars_categoricas, key="box_x")
                if var_x == "Ninguna":
                    variables = [var_y]
                else:
            ...
            var_y = st.selectbox("Variable numérica:", options=vars_numericas, key="violin_y")
            if vars_categoricas:
                var_x = st.selectbox("Variable categórica (opcional):", options=["Ninguna"] + vars_categoricas, key="violin_x")
                if var_x == "Ninguna":
                    variables = [var_y]
                else:
                    variables = [var_y, var_x]
            ...
            variables = [st.selectbox("Variable numérica:", options=vars_numericas, key="density_var")]
            linea_tendencia = False
    
    if variables and st.button("🎨 Crear Visualización", type="primary"):
        try:
            fig = crear_visualizacion_avanzada(
                df_actual, tipo_visualizacion, variables, 
