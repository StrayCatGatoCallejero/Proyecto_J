...
from pipeline_encuestas import (
    ...
    exportar_limpio,
    generar_reporte_pdf,
    pipeline_completo_encuestas,
    validar_archivo,
    # Nuevas funciones de validación de Chile
    validar_datos_chile,
    enriquecer_datos_chile_pipeline,
    analisis_regional_chile,
    comparar_nacional_chile,
...
def show_success_message(message: str):
    ...
    st.markdown(
        f'<div class="success-message fade-in">✅ {message}</div>',
        unsafe_allow_html=True,
    )
...
def show_error_message(message: str):
    ...
    st.markdown(
        f'<div class="error-message fade-in">❌ {message}</div>', unsafe_allow_html=True
    )
...
def show_warning_message(message: str):
    ...
    st.markdown(
        f'<div class="warning-message fade-in">⚠️ {message}</div>',
        unsafe_allow_html=True,
    )
...
def show_info_message(message: str):
    ...
    st.markdown(
        f'<div class="info-message fade-in">ℹ️ {message}</div>', unsafe_allow_html=True
    )
...
def show_instruction_text(message: str):
    ...
    st.markdown(
        f'<div class="instruction-text fade-in">💡 {message}</div>',
        unsafe_allow_html=True,
    )
...
def show_section_title(title: str):
    ...
    st.markdown(
        f'<h2 class="section-title fade-in">{title}</h2>', unsafe_allow_html=True
    )
...
def show_subsection_title(title: str):
    ...
    st.markdown(
        f'<h3 class="subsection-title fade-in">{title}</h3>', unsafe_allow_html=True
    )
...
def main():
    ...
    st.markdown(
        '<h1 class="main-header">📊 Proyecto J - Análisis de Encuestas</h1>',
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        st.markdown(
            '<h2 class="section-title">⚙️ Configuración</h2>', unsafe_allow_html=True
        )

        # Sección de carga de archivo
        st.markdown(
            '<h3 class="subsection-title">📁 Carga de Archivo</h3>',
            unsafe_allow_html=True,
        )
        ...
        if opcion_carga == "Subir archivo":
            show_instruction_text(
                "Carga un archivo en formato .CSV, .SAV o .DTA para comenzar. Tamaño máximo: 200MB."
            )
            ...
            show_instruction_text(
                "Selecciona un archivo que ya esté en la carpeta de datos del proyecto."
            )
            archivo_existente = st.selectbox(
                ...
                + [
                    ...
                    if f.endswith((".csv", ".sav", ".dta", ".xlsx", ".xls"))
                ],
            ...
            show_instruction_text(
                "Para archivos grandes (>200MB): Ingresa la ruta completa al archivo. No hay límite de tamaño."
            )
            st.markdown(
                """
        ...
        if archivo_subido is not None:
            ...
            if file_size > 200:
                show_warning_message(
                    "Archivo muy grande (>200MB). Considera usar 'Ruta local' para archivos grandes."
                )
            ...
            if file_size > 500:
                st.warning(
                    f"⚠️ Archivo muy grande ({file_size:.1f} MB). El procesamiento puede tardar varios minutos."
                )

        # Configuración de procesamiento
        st.markdown(
            '<h3 class="subsection-title">🔧 Opciones de Procesamiento</h3>',
            unsafe_allow_html=True,
        )

        show_instruction_text(
            "Elige cómo quieres que se procesen los datos faltantes en tu archivo."
        )
        ...
        with st.expander("🔬 Opciones Avanzadas"):
            show_instruction_text(
                "Estas opciones son recomendadas solo si tienes experiencia técnica. Si no estás seguro, deja los valores por defecto."
            )
            ...
            if ruta_local and os.path.exists(ruta_local):
                ...
                if file_size > 100:  # Solo mostrar para archivos grandes
                    st.markdown(
                        '<h4 class="subsection-title">📦 Procesamiento de Archivos Grandes</h4>',
                        unsafe_allow_html=True,
                    )
                    ...
                    if procesar_en_chunks:
                        ...
                        st.info(
                            f"💡 El archivo se procesará en partes de {chunk_size:,} filas"
                        )
            ...
            st.markdown(
                '<h4 class="subsection-title">📊 Visualización</h4>',
                unsafe_allow_html=True,
            )
    ...
    # Contenido principal
    if (
        archivo_subido is not None
        or (archivo_existente and archivo_existente != "Ninguno")
        or (ruta_local and os.path.exists(ruta_local))
    ):
        ...
        if not validate_file_format(ruta_archivo):
            show_error_message(
                "Formato de archivo no soportado. Use CSV, SAV, DTA, XLSX o XLS."
            )
            return
        ...
        show_instruction_text(
            "Aquí puedes ver un resumen rápido de tu archivo y explorar su contenido."
        )
                ...
                try:
                    with st.spinner("Ejecutando análisis completo..."):
                        ...
                        if usar_procesamiento_grande:
                            st.info(
                                f"📦 Usando procesamiento optimizado para archivo grande ({file_size:.1f} MB)"
                            )
        ...
        show_instruction_text(
            "Selecciona una variable para generar gráficos o explorar sus características."
        )
        ...
        show_instruction_text(
            "Valida y enriquece tus datos con información oficial de Chile."
        )
        ...
        with tab2:
            if st.button(
                "🔍 Validar Datos de Chile", disabled=st.session_state.pipeline_running
            ):
                try:
                    with st.spinner("Validando datos geográficos..."):
                        ...
                        if columna_region != "Ninguna" or columna_comuna != "Ninguna":
                            region_col = (
                                columna_region if columna_region != "Ninguna" else None
                            )
                            comuna_col = (
                                columna_comuna if columna_comuna != "Ninguna" else None
                            )

                            resultados_validacion = validar_datos_chile(
                                df, region_col, comuna_col
                            )
                            ...
                            if resultados_validacion["valido"]:
                                ...
                                show_error_message(
                                    "❌ Errores encontrados en validación geográfica"
                                )
        ...
        with tab3:
            if st.button(
                "📊 Enriquecer Datos", disabled=st.session_state.pipeline_running
            ):
                try:
                    with st.spinner("Enriqueciendo datos..."):
                        ...
                        if columna_region != "Ninguna" or columna_comuna != "Ninguna":
                            ...
                            df_enriquecido = enriquecer_datos_chile_pipeline(
                                df, region_col, comuna_col
                            )
                            ...
                            if "datos_enriquecidos" in st.session_state:
                                columnas_nuevas = set(df_enriquecido.columns) - set(
                                    df.columns
                                )
                                if columnas_nuevas:
                                    st.markdown(
        ...
        with tab4:
            if st.button(
                "📈 Analizar por Región", disabled=st.session_state.pipeline_running
            ):
                try:
                    if "datos_enriquecidos" in st.session_state:
                        ...
                        if variable_analizar:
                            with st.spinner("Analizando por región..."):
                                analisis_regional = analisis_regional_chile(
                                    df_enriquecido, variable_analizar
                                )
                                st.dataframe(analisis_regional)
                                show_success_message("✅ Análisis regional completado")
                    else:
                        show_warning_message(
                            "⚠️ Primero debes enriquecer los datos en la pestaña 'Enriquecimiento'"
                        )
        ...
        with tab5:
            if st.button(
                "🏛️ Comparar con Nacional", disabled=st.session_state.pipeline_running
            ):
                try:
                    if "datos_enriquecidos" in st.session_state:
                        df_enriquecido = st.session_state.datos_enriquecidos
                        ...
                        if variable_analizar:
                            with st.spinner("Comparando con promedios nacionales..."):
                                comparacion = comparar_nacional_chile(
                                    df_enriquecido, variable_analizar
                                )
                                st.dataframe(comparacion)
                                show_success_message(
                                    "✅ Comparación nacional completada"
                                )
                    else:
                        show_warning_message(
                            "⚠️ Primero debes enriquecer los datos en la pestaña 'Enriquecimiento'"
...
def mostrar_resultados_mejorados(resultados: Dict[str, Any]):
    ...
    show_instruction_text(
        "Cuando termines, podrás guardar los resultados en PDF, PNG o Excel."
    )
    ...
    with col4:
        if "datos_originales" in resultados and "datos_limpios" in resultados:
            valores_faltantes_originales = (
                resultados["datos_originales"].isnull().sum().sum()
            )
            valores_faltantes_limpias = resultados["datos_limpios"].isnull().sum().sum()
            valores_completados = (
                valores_faltantes_originales - valores_faltantes_limpias
            )
            st.markdown(
                f"""
            <div class="metric-card fade-in">
    ...
    # Microtexto final
    show_micro_text(
        "Los resultados se mantienen en memoria durante esta sesión. Descarga los archivos para guardarlos permanentemente."
    )
...
def mostrar_comparacion_datos(df_original: pd.DataFrame, df_limpio: pd.DataFrame):
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📋 Vista Previa", "📊 Estadísticas", "📈 Visualización"]
    )
    ...
    with tab3:
        ...
        fig = px.bar(
            null_comparison,
            x="Columna",
            y=["Faltantes Originales", "Faltantes Limpios"],
...
def mostrar_analisis_ponderado(df: pd.DataFrame):
    ...
    if len(numeric_cols) < 2:
        show_warning_message(
            "Se necesitan al menos 2 columnas numéricas para el análisis ponderado."
        )
        return
    ...
    with col1:
        variable = st.selectbox(
            "Variable a analizar", numeric_cols, key="var_ponderada"
        )
...
def mostrar_muestreo_estratificado(df: pd.DataFrame):
    ...
    columna_estratificacion = st.selectbox(
        "Columna para estratificación:", df.columns.tolist(), key="col_estratificacion"
    )
    ...
    with col2:
        random_state = st.number_input(
            "Semilla aleatoria", min_value=0, max_value=1000, value=42
        )

    if st.button(
        "📊 Realizar muestreo estratificado", disabled=st.session_state.pipeline_running
    ):
        try:
