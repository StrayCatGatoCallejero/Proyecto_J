...
# ============================================================================
# CLASIFICACIÓN AUTOMÁTICA DE VARIABLES - VERSIÓN ROBUSTA
# ============================================================================


class VariableClassifier:
    """
    Clasificador robusto de variables con detección automática de tipos
    y metadatos enriquecidos para ciencias sociales.
    """
    ...
    def classify_dataset(self, df: pd.DataFrame) -> dict:
        ...
        for col in df.columns:
            ...
            metadata = {
                ...
                'pct_unique': series.nunique(dropna=True) / n_rows,
                'sample_values': [str(val) for val in series.dropna().unique()[:5]]
            }
            
            # Aplicar capa de dominio de ciencias sociales
            social_metadata = self._apply_social_sciences_layer(col, series, metadata)
            metadata.update(social_metadata)
    ...
    def _classify_column(self, series: pd.Series, n_rows: int) -> str:
        """Clasifica una columna en tipos básicos."""
        # Detección temporal (solo si no es numérica)
        if not is_numeric_dtype(series) and self._detect_temporal(series):
            return 'temporal'
        # Numérica
    ...
    def _apply_social_sciences_layer(self, col: str, series: pd.Series, metadata: dict) -> dict:
        """Aplica la capa de clasificación específica de ciencias sociales."""
        valores = series.dropna()
        n_unicos = len(valores.unique())
        n_total = len(valores)
        
        # Detectar subtipos específicos
        es_ordinal = self._detect_ordinal(col, valores, metadata['type'])
        es_binaria = n_unicos == 2
        es_likert = self._detect_likert(col, valores, metadata['type'])
        es_continua = metadata['type'] == 'numerica' and not es_ordinal and not es_binaria
        
        # Clasificar dominio de ciencias sociales
        dominio = clasificar_dominio_cs(col, valores, metadata['type'] == 'categorica')
        ...
        return {
            ...
            'porcentaje_faltantes': (series.isna().sum() / len(series)) * 100,
        }

    def _detect_ordinal(self, col: str, valores, base_type: str) -> bool:
        """Detecta si una variable es ordinal."""
        if base_type != 'categorica':
            return False
        ...
        # Buscar patrones de orden en el nombre de la columna
        col_lower = col.lower()
        ordinal_keywords = ['nivel', 'grado', 'escala', 'rango', 'categoria', 'clase']
        if any(keyword in col_lower for keyword in ordinal_keywords):
            return True
            
        return False

    def _detect_likert(self, col: str, valores, base_type: str) -> bool:
        """Detecta si una variable es escala Likert."""
        if base_type != 'categorica':
            return False
            
        categorias = valores.unique()
        if len(categorias) in [3, 5, 7]:  # Escalas Likert típicas
            # Verificar si los valores son numéricos o contienen palabras clave
            try:
                # Si son numéricos, verificar rango típico de Likert
                valores_num = pd.to_numeric(valores, errors='coerce')
                if not valores_num.isna().all():
                    if valores_num.min() >= 1 and valores_num.max() <= 7:
                        return True
            except:
                pass
            ...
            col_lower = col.lower()
            likert_keywords = ['likert', 'acuerdo', 'satisfaccion', 'importancia', 'frecuencia']
            if any(keyword in col_lower for keyword in likert_keywords):
                return True
                
        return False


def clasificar_variables_avanzado(df: pd.DataFrame) -> dict:
    """
    Función principal para clasificar variables con metadatos enriquecidos.
    Reemplaza la función básica anterior.
    """
    clf = VariableClassifier(cat_threshold=0.1, cat_unique_limit=20)
    return clf.classify_dataset(df)
...
def clasificar_variable(df: pd.DataFrame, columna: str) -> dict:
    ...
    """
    clasificacion_completa = clasificar_variables_avanzado(df)
    return clasificacion_completa[columna]
...
def clasificar_dominio_cs(columna: str, valores, es_categorica: bool) -> str:
    ...
    if any(
        palabra in columna_lower for palabra in ["edad", "age", "años", "generacion"]
    ):
        return "Demográfico - Edad"
    elif any(
        palabra in columna_lower
        for palabra in ["genero", "sexo", "gender", "identidad"]
    ):
        return "Demográfico - Género"
    elif any(
        palabra in columna_lower for palabra in ["estado_civil", "civil", "matrimonio"]
    ):
        return "Demográfico - Estado Civil"
    elif any(
        palabra in columna_lower for palabra in ["nacionalidad", "pais", "ciudadania"]
    ):
    ...
    # Variables socioeconómicas
        ...
        palabra in columna_lower
        for palabra in ["ingreso", "salario", "renta", "income"]
    ):
        return "Socioeconómico - Ingresos"
    elif any(
        palabra in columna_lower
        for palabra in ["empleo", "trabajo", "ocupacion", "job"]
    ):
        return "Socioeconómico - Empleo"
    elif any(
        palabra in columna_lower for palabra in ["educacion", "escolaridad", "estudios"]
    ):
        return "Educativo"
    ...
    # Variables de salud
        ...
        palabra in columna_lower for palabra in ["religion", "religioso", "espiritual"]
    ):
        return "Religioso/Espiritual"
    elif any(palabra in columna_lower for palabra in ["politica", "voto", "partido"]):
        return "Político/Cívico"
    elif any(
        palabra in columna_lower for palabra in ["tecnologia", "internet", "digital"]
    ):
...
def analisis_descriptivo_cs(df: pd.DataFrame, columna: str) -> dict:
    ...
    if clasificacion["es_continua"]:
        resultado["estadisticas_basicas"] = {
            ...
            "rango_intercuartil": float(
                valores.quantile(0.75) - valores.quantile(0.25)
            ),
            "coef_variacion": (
                float(valores.std() / valores.mean()) if valores.mean() != 0 else 0
            ),
    ...
    resultado["interpretacion"] = interpretar_resultados_cs(
        columna, resultado, clasificacion
    )

    return resultado


def interpretar_resultados_cs(
    columna: str, resultado: dict, clasificacion: dict
) -> dict:
    """
    ...
    if clasificacion["dominio"] == "Demográfico - Edad":
        ...
        if clasificacion["es_continua"]:
            ...
            if media_ingreso > mediana_ingreso * 1.5:
                interpretacion["desigualdad"] = (
                    "Presencia de desigualdad (media > mediana)"
                )
            else:
...
def analisis_bivariado_cs(df: pd.DataFrame, var1: str, var2: str) -> dict:
    ...
    if clasif1["es_continua"] and clasif2["es_continua"]:
    ...
    elif (clasif1["es_continua"] and clasif2["es_categorica"]) or (
        clasif1["es_categorica"] and clasif2["es_continua"]
    ):
        ...
        resultado["analisis"] = analisis_grupos_continua(
            df_limpio, var_continua, var_categorica
        )
...
def analisis_grupos_continua(
    df: pd.DataFrame, var_continua: str, var_categorica: str
) -> dict:
    """
    ...
    if "correlacion_continua" in resultado["analisis"]:
        ...
        if pearson["significativo"]:
            interpretacion["conclusion"] = (
                f"Existe una correlación {pearson['fuerza'].lower()} y significativa entre las variables"
            )
            ...
            interpretacion["conclusion"] = (
                "No hay evidencia de correlación significativa entre las variables"
            )
        ...
        if analisis["chi_cuadrado"]["significativo"]:
            interpretacion["conclusion"] = (
                "Existe una relación significativa entre las variables categóricas"
            )
            ...
            interpretacion["conclusion"] = (
                "No hay evidencia de relación significativa entre las variables"
            )
...
def analisis_regresion_multiple_cs(
    df: pd.DataFrame, variable_dependiente: str, variables_independientes: list
) -> dict:
    """
    ...
    r2_ajustado = 1 - (1 - r2) * (len(y) - 1) / (
        len(y) - len(variables_independientes) - 1
    )
    ...
    for i, var in enumerate(variables_independientes):
        coeficientes[var] = {
            ...
            "coeficiente_estandarizado": float(
                modelo.coef_[i] * X[var].std() / y.std()
            ),
        }
...
def analisis_clusters_cs(
    df: pd.DataFrame, variables: list, n_clusters: int = 3
) -> dict:
    """
    ...
    for i in range(n_clusters):
        ...
        for var in variables:
            estadisticas_clusters[f"cluster_{i}"][f"media_{var}"] = float(
                cluster_data[var].mean()
            )
...
def calcular_indice_calidad_vida(
    df: pd.DataFrame, variables: list, pesos: list = None
) -> pd.Series:
    """
...
def validar_supuestos_regresion(
    df: pd.DataFrame, variable_dependiente: str, variables_independientes: list
) -> dict:
    """
    ...
    for i, var in enumerate(variables_independientes):
        ...
        if otras_vars:
            X_otras = X_scaled[
                :, [variables_independientes.index(v) for v in otras_vars]
            ]
            y_var = X_scaled[:, i]
...
def analizar_valores_perdidos(df: pd.DataFrame) -> dict:
            ...
            for otra_var in otras_vars:
                if df[otra_var].dtype in ["int64", "float64"]:
                    ...
                    patrones[col][otra_var] = {
                        ...
                        "diferencia": (
                            media_con_perdidos - media_sin_perdidos
                            if pd.notna(media_con_perdidos)
                            and pd.notna(media_sin_perdidos)
                            else None
                        ),
                    }
                else:
                    # Para variables categóricas, calcular frecuencias
                    frecuencias_con_perdidos = df.loc[
                        mascara_perdidos, otra_var
                    ].value_counts(normalize=True)
                    frecuencias_sin_perdidos = df.loc[
                        ~mascara_perdidos, otra_var
                    ].value_counts(normalize=True)
                    patrones[col][otra_var] = {
                        "tipo": "categorica",
    ...
    return {
        ...
        "porcentaje_total_perdidos": float(
            valores_perdidos.sum() / (len(df) * len(df.columns)) * 100
        ),
...
def sugerir_imputacion(df: pd.DataFrame, columna: str) -> dict:
    ...
    if clasificacion["es_continua"]:
        sugerencias["metodos_recomendados"].extend(
            ["Media", "Mediana", "Interpolación lineal", "Regresión múltiple"]
        )
    elif clasificacion["es_categorica"]:
        sugerencias["metodos_recomendados"].extend(
            [
    ...
    if sugerencias["porcentaje_perdidos"] > 50:
        sugerencias["advertencia"] = (
            "Alto porcentaje de valores perdidos. Considerar eliminar la variable."
        )
    elif sugerencias["porcentaje_perdidos"] > 20:
        sugerencias["advertencia"] = (
            "Porcentaje moderado de valores perdidos. Usar métodos de imputación con precaución."
        )
