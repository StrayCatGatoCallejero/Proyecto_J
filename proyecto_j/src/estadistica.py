...
def calcular_percentiles(
    df: pd.DataFrame, columna: str, percentiles: list = [25, 50, 75]
) -> dict:
...
def generar_heatmap_correlacion(
    df_corr: pd.DataFrame, titulo: str = "Matriz de Correlación"
) -> Figure:
    """
...
def generar_grafico_tabla_contingencia(
    df: pd.DataFrame, columna1: str, columna2: str
) -> Figure:
    """
...
def calcular_porcentajes_tabla_contingencia(
    df: pd.DataFrame, columna1: str, columna2: str
) -> dict:
    """
...
def crear_filtros_dinamicos(
    df: pd.DataFrame, columnas_numericas: list = None, columnas_categoricas: list = None
) -> dict:
    """
...
def generar_estadisticas_descriptivas_completas(
    df: pd.DataFrame, columnas_numericas: list = None
) -> pd.DataFrame:
    """
    ...
    for col in columnas_numericas:
        ...
        if len(valores) > 0:
            stats_dict = {
                ...
                "Coef. Variación": (
                    (valores.std() / valores.mean()) * 100 if valores.mean() != 0 else 0
                ),
            }
...
def generar_resumen_correlaciones(
    df: pd.DataFrame, variables: list, tipo_correlacion: str = "pearson"
) -> pd.DataFrame:
    """
    ...
    return pd.DataFrame(correlaciones).sort_values(
        "Correlación", key=abs, ascending=False
    )


def generar_resumen_tablas_contingencia(
    df: pd.DataFrame, variable1: str, variable2: str
) -> dict:
    """
...
def generar_html_reporte(
    df: pd.DataFrame,
    filtros: dict,
    estadisticas_descriptivas: pd.DataFrame = None,
    correlaciones: pd.DataFrame = None,
    tablas_contingencia: dict = None,
) -> str:
    """
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for col, filtro in filtros.items():
            if isinstance(filtro, dict):
                ...
                resumen_filtros.append(
                    {"Variable": col, "Tipo": "Categorías", "Valor": ", ".join(filtro)}
                )

        if resumen_filtros:
            pd.DataFrame(resumen_filtros).to_excel(
                writer, sheet_name="Resumen_Filtros", index=False
            )

        # Hoja 3: Estadísticas descriptivas
        if estadisticas_descriptivas is not None:
            estadisticas_descriptivas.to_excel(
                writer, sheet_name="Estadisticas_Descriptivas", index=False
            )
        ...
        if tablas_contingencia is not None:
            tablas_contingencia["tabla_contingencia"].to_excel(
                writer, sheet_name="Tabla_Contingencia"
            )
            tablas_contingencia["porcentajes_fila"].to_excel(
                writer, sheet_name="Porcentajes_Fila"
            )
            tablas_contingencia["porcentajes_columna"].to_excel(
                writer, sheet_name="Porcentajes_Columna"
            )
            ...
            resultados_df = pd.DataFrame(
                [
                    ...
                    {"Estadístico": "p-valor", "Valor": resultados["p_value"]},
                    {
                        ...
                        "Valor": resultados["degrees_of_freedom"],
                    },
                    {"Estadístico": "Cramer's V", "Valor": resultados["cramer_v"]},
                    {
                        "Estadístico": "Coeficiente de contingencia",
                        "Valor": resultados["pearson_c"],
                    },
                ]
            )
            resultados_df.to_excel(
                writer, sheet_name="Resultados_Chi_Cuadrado", index=False
            )
...
def crear_grafico_plotly_correlacion(
    df_corr: pd.DataFrame, titulo: str = "Matriz de Correlación"
):
    """
...
def generar_boxplot(
    df: pd.DataFrame, columna_numerica: str, columna_categorica: str = None
):
    """
...
def generar_scatter_plot(
    df: pd.DataFrame, columna_x: str, columna_y: str, columna_color: str = None
):
    """
    ...
    if columna_color:
        ...
        plt.title(
            f"Scatter Plot: {columna_x} vs {columna_y} (coloreado por {columna_color})"
        )
    else:
        # Scatter plot simple
...
def generar_diagrama_densidad(
    df: pd.DataFrame, columna: str, columna_grupo: str = None
):
    """
...
def generar_grafico_barras(
    df: pd.DataFrame, columna_categorica: str, columna_numerica: str = None
):
    """
    ...
    if columna_numerica:
        ...
        df_agrupado = (
            df.groupby(columna_categorica)[columna_numerica]
            .mean()
...
def generar_histograma_densidad(
    df: pd.DataFrame, columna: str, columna_grupo: str = None
):
    """
    ...
    if columna_grupo:
        ...
        # Histograma simple
        plt.hist(
            df[columna].dropna(), bins=20, density=True, alpha=0.7, color="skyblue"
        )
...
def generar_violin_plot(
    df: pd.DataFrame, columna_numerica: str, columna_categorica: str
):
    """
    ...
    plt.title(
        f"Violin Plot: Distribución de {columna_numerica} por {columna_categorica}"
    )
    plt.xticks(rotation=45)
    plt.tight_layout()
...
def generar_heatmap_correlacion_avanzado(df: pd.DataFrame, variables: list):
    ...
    ax2.barh(
        range(len(variables_corr)),
        [abs(x) for x in valores_corr],
        color=colors,
        alpha=0.7,
    )
...
def generar_panel_visualizaciones(
    df: pd.DataFrame, columna_principal: str, columna_grupo: str = None
):
    """
    ...
    if columna_grupo:
        for grupo in df[columna_grupo].unique():
            ...
            if len(datos_grupo) > 0:
                axes[0, 0].hist(
                    datos_grupo, alpha=0.5, label=grupo, density=True, bins=20
                )
        axes[0, 0].legend()
    else:
        axes[0, 0].hist(
            df[columna_principal].dropna(), bins=20, density=True, alpha=0.7
        )
