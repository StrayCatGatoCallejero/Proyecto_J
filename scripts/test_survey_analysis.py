...
def generate_synthetic_survey_data(n_samples=100):
    ...
    income_levels = np.random.choice([
        'Menos de $300,000', '$300,000-$500,000', '$500,000-$800,000', 
        '$800,000-$1,200,000', 'Más de $1,200,000'
    ], n_samples, p=[0.20, 0.30, 0.25, 0.15, 0.10])
    ...
    ], n_samples, p=[0.30, 0.20, 0.15, 0.15, 0.10, 0.10])
    ...
    ], n_samples, p=[0.40, 0.15, 0.10, 0.10, 0.15, 0.10])
...
def test_individual_functions():
    ...
    if 'error' not in text_analysis:
        ...
        print(f"   ✅ Palabras más frecuentes: {len(text_analysis['word_cloud'])} palabras")
    else:
...
def test_survey_analysis():
    ...
    if success:
        ...
        # Basic info
        print(f"📋 Datos: {session_data.df.shape[0]} filas × {session_data.df.shape[1]} columnas")
        print(f"🔍 Puede visualizar: {session_data.metadata.get('can_visualize', False)}")
        ...
        if 'textual_summaries' in session_data.reports:
            ...
            for col_name, text_data in text_summaries.items():
                ...
                print(f"   - {col_name}: {sentiment['positive_pct']}% positivo, "
        ...
        output_path = f"reporte_encuesta_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
