...
from proyecto_j.src.pipeline_encuestas import (
    cargar_datos,
    explorar_datos,
    visualizar_nulos,
    imputar_nulos,
...
class TestImputarNulos:
    ...
    def test_columnas_especificas(self):
        ...
        df_limpio = imputar_nulos(
            self.df, estrategia="media", columnas_numericas=["numerica"]
        )
        assert df_limpio["numerica"].isnull().sum() == 0
...
class TestAnalisisPonderado:
    ...
    def setup_method(self):
        """Configuración inicial para cada prueba."""
        self.df = pd.DataFrame(
            {"variable": [10, 20, 30, 40, 50], "peso": [1.0, 1.2, 0.8, 1.1, 0.9]}
        )
    ...
    def test_analisis_ponderado_peso_inexistente(self):
        ...
        with pytest.raises(
            ValueError, match="Variable de peso 'inexistente' no encontrada"
        ):
    ...
    def test_analisis_ponderado_datos_vacios(self):
        ...
        df_vacio = pd.DataFrame({"variable": [np.nan, np.nan], "peso": [1.0, 1.0]})
...
class TestMuestreoEstratificado:
    ...
    def test_muestreo_estratificado_columna_inexistente(self):
        ...
        with pytest.raises(
            ValueError, match="Columna de estratificación 'inexistente' no encontrada"
        ):
    ...
    def test_muestreo_estratificado_diferentes_test_sizes(self):
        ...
        for test_size in [0.1, 0.2, 0.3]:
            train_df, test_df = muestreo_estratificado(
                self.df, "estrato", test_size=test_size
            )
...
class TestExportarLimpio:
    ...
    def setup_method(self):
        """Configuración inicial para cada prueba."""
        self.df = pd.DataFrame({"columna1": [1, 2, 3], "columna2": ["A", "B", "C"]})
        self.temp_dir = tempfile.mkdtemp()
...
class TestGenerarReportePDF:
    ...
    def test_generar_reporte_pdf_exitoso(self, mock_run):
        ...
        # En modo test, la función detecta pytest y retorna la ruta directamente
        # sin llamar a subprocess.run, por lo que no verificamos que se llame
