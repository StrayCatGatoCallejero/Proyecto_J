...
def test_individual_functions():
    ...
    summary = crosstab_summary(df, "genero", "nivel_educacion")
...
def test_survey_analysis():
    ...
    orchestrator.run_pipeline(temp_file)

    session_data = orchestrator.get_pipeline_results()
    ...
    # Basic info
    data_shape = session_data['data_info']['shape']
    if data_shape:
        print(f"📋 Datos: {data_shape[0]} filas × {data_shape[1]} columnas")
    else:
        print("📋 Datos: No disponibles")
    print(
        f"🔍 Puede visualizar: {len(session_data['visualizations']) > 0}"
    )
