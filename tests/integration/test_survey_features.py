...
def summarize_survey_structure(df, metadata):
        ...
        for semantic_type, count in type_counts.items():
            if semantic_type == "demographic":
                type_descriptions.append(
                    f"{count} demográfica{'s' if count > 1 else ''}"
                )
    ...
    if total_missing > 0:
        narrative_parts.append(
            f" El {avg_missing:.1f}% de los campos están vacíos en promedio."
        )
...
def crosstab_summary(df, col):
    ...
    summary_parts.append(
        f"La opción '{most_freq['Valor']}' fue la más frecuente ({most_freq['Porcentaje']}%)"
    )
    ...
    if len(top_values) > 1:
        ...
        summary_parts.append(
            f", seguida de '{second_freq['Valor']}' ({second_freq['Porcentaje']}%)"
        )
    ...
    if len(top_values) > 2:
        ...
        summary_parts.append(
            f" y '{third_freq['Valor']}' ({third_freq['Porcentaje']}%)"
        )
...
def textual_summary(df, col):
    ...
    return {
        ...
        "avg_response_length": round(
            np.mean([len(str(text)) for text in text_data]), 1
        ),
    }
